...
const LABEL = {
  ...
  bank_deposit: 'Reserved money', bank_withdraw: 'Reserve taken back', wastage: 'Wastage',
...
export default function Page() {
  ...
  // Keep the balance live — the counter phone and the office see the same figure.
  useEffect(() => {
  ...
  live.filter((t) => t.date <= date && bankMove(t) !== 0).forEach((t) => {
    const n = t.category || 'Unnamed';
    reserveByName[n] = (reserveByName[n] || 0) + bankMove(t);
  });
...
function Home(c) {
  ...
  return (
    <>
      <div className="card">
        <span className="lbl">Cash in drawer · {dshow(c.date)}
          <span style={{ color: 'var(--leaf)', marginLeft: 8 }}>● live</span></span>
      ...
      <div className="card">
        ...
        <Row label="Purchase — cash" value={c.purchCash} type="purchase_cash" />
        <Row label="Purchase — credit" value={c.purchCredit} type="purchase_credit" colour="var(--mango)" />
...
function DayList({ c, type, close }) {
  const filters = {
    ...
    purchase_cash: (t) => t.type === 'purchase' && t.mode === 'cash',
    purchase_credit: (t) => t.type === 'purchase' && t.mode === 'credit',
  };
  const titles = {
    cash_in_all: 'Cash in', cash_out_all: 'Cash out',
    purchase_cash: 'Purchases paid in cash', purchase_credit: 'Purchases on credit',
  };
...
function TxnForm({ c, type, close }) {
  ...
  const save = async () => {
    ...
    await c.run('entry', {
      ...
      category: (type === 'expense' || type === 'bank_deposit' || type === 'bank_withdraw')
        ? (category === CATS[0] && type !== 'expense' ? 'Unnamed' : category)
        : type === 'wastage' ? (remarks || 'wastage') : null,
      ref, remarks,
    }, null, true);
    setBusy(false);
  ...
  return (
    <div className="sheet" onClick={close}>
      <div className="sheetin" onClick={(e) => e.stopPropagation()}>
        ...
        {(type === 'bank_deposit' || type === 'bank_withdraw') && (
          <div className="f"><label className="lbl">Kept with — bank or person</label>
            <input value={category === CATS[0] ? '' : category} placeholder="e.g. SBI current, or Abi"
...
/* --------------------------- balance adjustment --------------------------- */
function AdjustForm({ c, close }) {
  const [target, setTarget] = useState('cash');
  const [name, setName] = useState('');
  const [correct, setCorrect] = useState('');
  const [reason, setReason] = useState('');

  const current = target === 'cash' ? c.expectedCash : c.bankTotal;
  const wanted = correct === '' ? null : parseFloat(correct);
  const gap = wanted === null || isNaN(wanted) ? 0 : wanted - current;
  const ok = gap !== 0 && reason.trim().length >= 3;

  return (
    <div className="sheet" onClick={close}>
      <div className="sheetin" onClick={(e) => e.stopPropagation()}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
          <div><div className="brand" style={{ fontSize: 18 }}>Adjust balance</div>
            <div className="sub">admin only · {dshow(c.date)}</div></div>
          <button className="pill" onClick={close}>Close</button>
        </div>

        <div className="f"><label className="lbl">What are you correcting?</label>
          <div className="tabs" style={{ padding: 0 }}>
            {[['cash', 'Cash in drawer'], ['reserve', 'Reserve']].map(([k, l]) => (
              <button key={k} type="button" className={'tab' + (target === k ? ' on' : '')}
                onClick={() => setTarget(k)}>{l}</button>))}
          </div></div>

        {target === 'reserve' && (
          <div className="f"><label className="lbl">Held with</label>
            <input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. SBI current, or Abi" /></div>
        )}

        <div className="card" style={{ marginTop: 0 }}>
          <div className="rowb"><span className="k">App shows now</span><span className="v">{money(current)}</span></div>
          <div className="rowb"><span className="k">You are setting it to</span>
            <span className="v">{wanted === null ? '—' : money(wanted)}</span></div>
          <div className="rowb"><b className="k" style={{ color: 'var(--chalk)' }}>Correction</b>
            <b className="v" style={{ fontSize: 18, color: gap > 0 ? 'var(--leaf)' : gap < 0 ? 'var(--beet)' : 'var(--muted)' }}>
              {gap > 0 ? '+' : ''}{money(gap)}</b></div>
        </div>

        <div className="f" style={{ marginTop: 14 }}><label className="lbl">Correct amount right now</label>
          <input inputMode="decimal" autoFocus value={correct} placeholder="0"
            onChange={(e) => setCorrect(e.target.value)}
            style={{ fontFamily: 'var(--mono)', fontSize: 26, padding: '16px 14px' }} /></div>

        <div className="f"><label className="lbl">Reason — required</label>
          <input value={reason} onChange={(e) => setReason(e.target.value)}
            placeholder="e.g. sale of 14th entered twice" /></div>

        <button className="btn" disabled={!ok}
          onClick={() => c.run('adjust', { target, name, gap, reason, date: c.date },
            'Balance adjusted')}>
          Adjust by {gap > 0 ? '+' : ''}{money(gap)}
        </button>
        <p className="empty" style={{ fontSize: 12 }}>
          Nothing is erased. The correction is written as its own entry with your reason,
          visible in the ledger and the activity log.
        </p>
      </div>
    </div>
  );
...
function Ledger({ c }) {
  ...
  const from = useMemo(() => {
    ...
    if (isNaN(d.getTime()) || range === 'day') return c.date;
    if (range === 'week') { d.setDate(d.getDate() - 6); return d.toISOString().slice(0, 10); }
    d.setDate(1); return d.toISOString().slice(0, 10);
  }, [range, c.date]);

  // Every entry, nothing hidden — deposits, withdrawals, cash in and out included.
  const rows = c.live.filter((t) => t.date >= from && t.date <= c.date)
    .slice().sort((a, b) => (a.date === b.date ? a.id - b.id : a.date < b.date ? -1 : 1));

  const openBefore = c.live.filter((t) => t.date < from)
    .reduce((a, t) => a + (['bank_deposit', 'bank_withdraw'].includes(t.type) ? 0 : cashEffect(t)), 0);

  let running = openBefore;
  const lines = rows.map((t) => {
    const move = ['bank_deposit', 'bank_withdraw'].includes(t.type) ? 0 : cashEffect(t);
    running += move;
    return { t, move, bal: running, transfer: bankMove(t) };
  });

  const share = async () => {
    const text = lines.map((l) =>
      `${dshow(l.t.date)} ${LABEL[l.t.type]} ${l.move >= 0 ? '+' : '-'}${Math.abs(l.move)} bal ${l.bal}`).join('\n');
    try {
      if (navigator.share) await navigator.share({ text });
      else { await navigator.clipboard.writeText(text); c.say('Ledger copied'); }
    } catch { /* cancelled */ }
  };
  ...
  return (
    <>
      ...
      <div className="card">
        <span className="lbl">{dshow(from)} → {dshow(c.date)} · {lines.length} entries</span>
        <div className="rowb"><span className="k">Balance before</span><span className="v">{money(openBefore)}</span></div>
        <div className="rowb"><b className="k" style={{ color: 'var(--chalk)' }}>Balance after</b>
          <b className="v" style={{ fontSize: 19 }}>{money(running)}</b></div>
      </div>

      <div className="card">
        {lines.length === 0 ? <p className="empty">No entries in this period.</p> : lines.map((l) => {
          const party = c.parties.find((p) => p.id === l.t.party_id);
          return (
            <div className="item" key={l.t.id}>
              <div style={{ minWidth: 0 }}>
                <b style={{ fontSize: 14 }}>{LABEL[l.t.type]}</b>
                <small>{dshow(l.t.date)} · {party ? party.name : l.t.category || l.t.remarks || '—'}
                  {' · '}{l.t.mode}{l.t.ref_no ? ' · ' + l.t.ref_no : ''} · {l.t.created_by}</small>
              </div>
              <div style={{ textAlign: 'right' }}>
                <div className="v" style={{ color: l.transfer ? 'var(--mango)' : l.move > 0 ? 'var(--leaf)' : l.move < 0 ? 'var(--beet)' : 'var(--muted)' }}>
                  {l.transfer ? (l.transfer > 0 ? '→ reserve ' : '← reserve ') + money(Math.abs(l.transfer))
                    : (l.move > 0 ? '+' : l.move < 0 ? '−' : '') + money(Math.abs(l.move) || l.t.amount)}
                </div>
                <small style={{ color: 'var(--muted)', fontFamily: 'var(--mono)', fontSize: 11 }}>{money(l.bal)}</small>
              </div>
            </div>
          );
        })}
      ...
      <button className="btn ghost" onClick={share}>Share this ledger</button>
      <p className="empty" style={{ fontSize: 12 }}>
        Every entry appears here. Reserve moves are marked in orange — they shift money between
        the drawer and a reserve without changing the total in hand.
      </p>
    </>
  );
}
  ...
  return (
    <>
      ...
      <div className="card">
        ...
        {(() => {
          const adj = c.live.filter((t) => (t.remarks || '').startsWith('Adjustment:') || t.category === 'Adjustment')
            .slice().sort((a, b) => (a.date < b.date ? 1 : -1)).slice(0, 15);
          return adj.length === 0
            ? <p className="empty">No adjustments made.</p>
            : adj.map((t) => (
              <div className="item" key={t.id}>
                <div><b style={{ fontSize: 13 }}>{(t.remarks || '').replace('Adjustment: ', '')}</b>
                  <small>{dshow(t.date)} · by {t.created_by}</small></div>
                <span className="v" style={{ color: cashEffect(t) > 0 ? 'var(--leaf)' : 'var(--beet)' }}>
                  {cashEffect(t) > 0 ? '+' : '−'}{money(t.amount)}</span>
              </div>
            ));
        })()}
      </div>
