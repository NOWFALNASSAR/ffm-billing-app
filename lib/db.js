...
// Connect on the first query, not when this file loads.
// During a build there is no DATABASE_URL yet, and neon() would throw.
let _sql;
function client() {
  if (!_sql) {
    if (!process.env.DATABASE_URL) throw new Error('DATABASE_URL is not set in this environment.');
